...
export const env = {
  ...raw,
  // Pool ceiling. Explicit DB_POOL_MAX wins; otherwise 8 on a Supabase pooler
  // (free tier: 15 per project, shared by every client) and 20 elsewhere. The
  // Supabase default lives in code, not only in render.yaml, because a new
  // blueprint variable does not always reach a running service unprompted.
  poolMax: raw.DB_POOL_MAX ?? (/\.supabase\.(com|co)$/i.test(hostOf(raw.DATABASE_URL)) ? 8 : 20),
